  ...
  const flattenLiveMatches = (payload) => {
      ...
      payload.typeMatches.forEach((tm) => {
        ...
        if (Array.isArray(series)) {
          series.forEach((s) => {
            ...
            else if (Array.isArray(s.seriesMatches))
              out.push(...s.seriesMatches);
    ...
    out.forEach((m) => {
      const id =
        ...
        m?.unique_id ||
        m?.mid ||
        (m?.matchInfo && m.matchInfo.matchId) ||
  ...
  const fetchInitialLive = useCallback(async () => {
    ...
    try {
      ...
      if (matches && matches.length > 0)
        setSelectedMatch(String(matches[0].id));
    } catch (err) {
